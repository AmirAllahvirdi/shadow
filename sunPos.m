 function sun_CT=sunPos(sp3)
%% PURPOSE: COMPUTE SUN POSITION AT ECEF FRAME(CT)
% INPUT:
%         sp3 :          struct contains sp3 data
% OUTPUT:
%         SUN_CT:        SUN POSITION
%
% AMIR ALLAHVERDI ZADEH(amir.allahvirdizadeh@curtin.edu.au)
% DEC 2011
%% compute julian ephemerids day,julian ephemerids century,julian ephemerids millenium
format long g
for i=1:size(sp3.data,2)
t_input(i,:)=[sp3.data(i).Epoch(1),sp3.data(i).Epoch(2),sp3.data(i).Epoch(3),sp3.data(i).Epoch(4),sp3.data(i).Epoch(5),sp3.data(i).Epoch(6)];
end
if ~isstruct(t_input)
    time.UTC=0;
    time.year = t_input(:,1);
    time.month = t_input(:,2);
    time.day = t_input(:,3);
    time.hour = t_input(:,4);
    time.min = t_input(:,5);
    time.sec = t_input(:,6);
...
%% compute the earth position relative to the sun, using tabulated values. 
...
for i=1:length(JME)
L0(i,1) = sum(A0 .* cos(B0 + (C0 * JME(i))));
end
for i=1:length(JME)
L1(i,1) = sum(A1 .* cos(B1 + (C1 * JME(i))));
end
for i=1:length(JME)
L2(i,1) = sum(A2 .* cos(B2 + (C2 * JME(i))));
end
for i=1:length(JME)
L3(i,1) = sum(A3 .* cos(B3 + (C3 * JME(i))));
end
for i=1:length(JME)
L4(i,1) = sum(A4 .* cos(B4 + (C4 * JME(i))));
end
for i=1:length(JME)
L5(i,1)= A5 .* cos(B5 + (C5 * JME(i)));
end
...
%% Compute the nutation in longtitude and in obliquity, in degrees.
...
for i=1:length(JCE)
delta_longitude(:,i) = ((nutation_terms(:,1) + (nutation_terms(:,2) * JCE(i)))) .* sin(tabulated_argument(:,i));
end
for i=1:length(JCE)
delta_obliquity(:,i) = ((nutation_terms(:,3) + (nutation_terms(:,4) * JCE(i)))) .* cos(tabulated_argument(:,i));
end
...
%% compute the sun rigth ascension. 
...
%% compute sun geocentric declination
...
%% compute observer_local_hour
location.X=0;
location.Y=0;
location.Z=0;
observer_local_hour = apparent_stime_at_greenwich + location.Y - sun_rigth_ascension;
...
u = atan(0.99664719 * tan(location.X * pi/180));
% Term x, used in the following calculations
x = cos(u) + ((location.Z/6378140) .* cos(location.X * pi/180));
% Term y, used in the following calculations
y = (0.99664719 * sin(u)) + ((location.Z/6378140) .* sin(location.X * pi/180));
...
sun_RA.X=cosd(topocentric_sun_position.declination).*cosd(topocentric_sun_position.rigth_ascension);
sun_RA.Y=-cosd(topocentric_sun_position.declination).*sind(topocentric_sun_position.rigth_ascension);
sun_RA.Z=sind(topocentric_sun_position.declination);
S_GHA=mean_stime-topocentric_sun_position.rigth_ascension;
S_GHA=set_to_range(S_GHA, 0, 360);
sun_CT.Time=t_input;
sun_CT.X=cosd(topocentric_sun_position.declination).*cosd(S_GHA);
sun_CT.Y=- cosd(topocentric_sun_position.declination).*sind(S_GHA);
sun_CT.Z=sind(topocentric_sun_position.declination);
